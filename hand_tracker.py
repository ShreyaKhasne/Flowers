...
class HandTracker:
    ...
    def _init_mediapipe(self):
        ...
        try:
            # 1. Try MediaPipe Tasks API (MediaPipe 1.0.0+)
        ...
        try:
            # 2. Try Legacy MediaPipe Solutions API (MediaPipe < 1.0)
            import mediapipe as mp
    ...
    def process_frame(self, target_width, target_height):
        ...
        if self.use_tasks_api and self.detector is not None:
            # MediaPipe Tasks API 1.0+ Processing
            ...
            # Legacy MediaPipe Solutions Processing
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    ...
    def _format_hand_info(self, hand_idx, handedness, raw_pts, target_width, target_height, dt):
        """Applies EMA smoothing and extracts gesture landmarks."""
        ...
        # Open Palm Detection
        fingertip_dists = [
            math.hypot(pt[0] - wrist[0], pt[1] - wrist[1])
            for pt in [thumb_tip, index_tip, middle_tip, ring_tip, pinky_tip]
        ]
        avg_tip_dist = sum(fingertip_dists) / 5.0
        is_open_palm = (avg_tip_dist / hand_scale) > 1.35
